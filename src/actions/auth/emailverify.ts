...
import { GetEmailTokenByTOKEN, GetUserByEMAIL } from "@/lib/get";
...
export async function EMAILVERIFY(
    ...
    const validated = schema.safeParse(Object.fromEntries(formData)); // Validate form data against zod schema
    ...
    await prisma.user.update({
        where: { id: user.id },
        data: { email: token.email, verified: new Date() }
    });
